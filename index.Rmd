...
final_summary <- all_data %>%
  ...
  summarise(total_kw_fortis_from_grid  = sum(kw_fortis_from_grid, na.rm = TRUE),
            total_kw_solar_to_grid = sum(kw_solar_to_grid, na.rm = TRUE),
            total_kw_solar_gross_produced = sum(kw_solar_gross_produced, na.rm = TRUE),
            total_kw_solar_used = sum(kw_solar_gross_produced - kw_solar_to_grid, na.rm = TRUE),
            total_kw_consumed = sum(kw_solar_gross_produced - kw_solar_to_grid + kw_fortis_from_grid, na.rm = TRUE))
```
...
For simplicity, we'll use a rate of $0.13 / kWh to calculate our cost savings.

Eventually I'll use the actual rates to calculate more accurate numbers.
...
net_cost <- round(0.13266*(final_summary$total_kw_fortis_from_grid - final_summary$total_kw_solar_to_grid), 2)
