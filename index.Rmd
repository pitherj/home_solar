...
```{r}
kbl(tot3, caption = "Summary of energy production and consumption from July 28 to Sept 28 2022.", 
    booktabs = TRUE, digits = c(2, 2, 2, 2, 2)) %>% 
  kable_styling(latex_options = c("scale_down", "hold_position"), position = "center")
```
...
```{r}
kbl(tot4, caption = "Summary of energy production and consumption from Sept 29 to Nov 28 2022.", 
    booktabs = TRUE, digits = c(2, 2, 2, 2, 2)) %>% 
  kable_styling(latex_options = c("scale_down", "hold_position"), position = "center")
```
...
```{r}
kbl(tot5, caption = "Summary of energy production and consumption from Nov 29 2022 to Jan 28 2023.", 
    booktabs = TRUE, digits = c(2, 2, 2, 2, 2)) %>% 
  kable_styling(latex_options = c("scale_down", "hold_position"), position = "center")
```
...
So let's do it for the entire period now (through Feb 10):

```{r, echo = FALSE}
final_summary <- all_data %>%
  filter(day >= ymd("2022-07-15") & day <= ymd("2023-02-10")) %>%
  summarise(fortis_consumed  = sum(kw_consumed, na.rm = TRUE),
            fortis_returned = sum(kw_returned, na.rm = TRUE),
            solar_gross_kWh_produced = sum(kWh, na.rm = TRUE),
            solar_kWh_used = sum(kWh - kw_returned, na.rm = TRUE),
