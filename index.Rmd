...
* gross solar energy we've produced (`total_solar_produced`)
* total energy consumed from grid (`total_fortis_consumed`)
* total solar energy returned to grid (`total_solar_returned`)

```{r totnums, echo = FALSE}
all_data %>%
...
all_data %>%
  ...
  summarise(fortis_consumed  = sum(kw_consumed, na.rm = TRUE),
            ...
            total_energy_consumed = sum(solar_kWh_used + fortis_consumed, na.rm = TRUE))
```
...
So let's do it for the entire period now (through Jan 28):
