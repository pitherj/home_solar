...
```{r echo = FALSE}
latest.bill.bank.number <- 1702
```

Our latest bill (from Nov 17 2023 to March 16, 2024) shows our net metering "bank" has a balance of `r latest.bill.bank.number` kWh.  

```{r}
new_net <- all_data %>%
  filter(day > "2024-01-16") %>%
...
Last year from March 17 through March 31st we used:

```{r}
winter_usage <- all_data %>%
  filter(day > "2023-03-16" & day < "2023-04-01") %>%
  summarise(excess_kWh  = sum(kw_solar_gross_produced, na.rm = TRUE) -  (sum(kw_fortis_from_grid, na.rm = TRUE) + sum(kw_consumed_from_solar, na.rm = TRUE)))
abs(winter_usage)
```

So we should have `r winter_usage + latest.bill.bank.number` buffer in banked hours.  So we should get a net-metering refund of  `r round((winter_usage + latest.bill.bank.number) * 0.1, 2)` dollars.
